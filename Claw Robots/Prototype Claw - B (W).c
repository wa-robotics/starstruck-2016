...
task autonStars() {
	...
	if (bIfiAutonomousMode) {
		wait1Msec(1500); //in an actual match (or with competition switch), delay for a bit to prevent collisions
	}
	driveDistancePID(-300,STRAIGHT,750);
	...
	liftTime = 750;
	liftgo = 1;
	startTask(asyncLiftPID); //raising the lift prevent polycarb from dragging when strafing
	...
	driveDistancePID(300,STRAFE,750);
	liftToBottom(); //put the lift down
